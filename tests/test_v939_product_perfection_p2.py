...
def test_client_reference_grid_keeps_route_and_matches_left_with_pick_on_the_right():
    ...
    continuation = template.index('data-v939-layout-contract="full-width-continuation"', primary)
    featured_pick = template.index('class="ns16-section ns16-featured-pick"', continuation)
    quick_actions = template.index('class="ns16-home-actions"', featured_pick)

    assert bounded < primary < continuation < featured_pick < quick_actions
    assert 'data-sports-priority="important-today"' in template[primary:featured_pick]
    assert "'/picks'" in template[featured_pick:quick_actions]
...
def _timestamp_contract_environment() -> Environment:
    ...
    environment.globals["ui"] = lambda value, *args, **kwargs: str(value).format(*args, **kwargs)
    return environment

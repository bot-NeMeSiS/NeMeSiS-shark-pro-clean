...
def chromium():
    ...
    with sync_playwright() as pw:
        ...
        executable = os.environ.get('NEMESIS_QA_CHROMIUM') or shutil.which('chromium')
...
def screen(chromium):
    ...
    page.add_script_tag(content=(ROOT/'static/v934-realtime.js').read_text(encoding='utf-8'))
...
def test_open_card_updates_in_place_and_preserves_navigation_and_focus(screen):
    ...
    assert page.locator('[data-directo-card-notice]').get_attribute('data-change-kind') == 'SCORE_UPDATE'
    assert 'No confirma un evento de gol' in page.locator('[data-directo-card-notice]').inner_text()
...
def test_duplicate_script_does_not_create_duplicate_pollers(screen):
    page,t=screen;page.add_script_tag(content=(ROOT/'static/v934-realtime.js').read_text(encoding='utf-8'))
    advance(page,45000)

...
require(version in {VERSION, V778_VERSION}, f"VERSION.txt debe ser V777/V778 compatible, es {version}")
app = read("app.py")
require(f'APP_VERSION = "{VERSION}"' in app or f'APP_VERSION = "{V778_VERSION}"' in app, "APP_VERSION no actualizado")
...
for token in (["Centro de mando", "Ruta recomendada", "Focos del día", "Picks claros", "Sin inventar"] if version.startswith("V778") else ["Centro de mando", "Ruta recomendada", "Partidos/focos", "Picks claros", "Sin inventar"]):
...
for token in (["@media(max-width:760px)", "V778_CLIENT_PRODUCT_ORGANIZATION_MADRID_TIME_FINAL_STABILITY", ".v778-home-hero"] if version.startswith("V778") else ["V777_CLIENT_PRODUCT_EXPERIENCE_FINAL_SYSTEM", ".v777-client-rail", ".v777-intent-grid", "@media (max-width: 760px)"]):

...
REPORTS = [
    ...
    "V870_REFERENCE_STYLE_MATCH_AND_WORKSPACE_PURGE_PRO_MAX_REPORT.md",
    "V870_WORKSPACE_FULL_TREE_AND_HIDDEN_FILES_AUDIT.md",
    "V870_WORKSPACE_PURGE_PRO_MAX_PLAN.md",
    "V870_VIDEO_REAL_VS_REFERENCE_STYLE_AUDIT.md",
    "V870_CLIENT_PC_REFERENCE_DASHBOARD_QA.md",
    "V870_MOBILE_NATIVE_REFERENCE_QA.md",
    "V870_ADMIN_COMMAND_CENTER_REFERENCE_QA.md",
    "V870_SPORTS_PICKS_LIVE_RICHNESS_QA.md",
    "V870_SHARK_TELEGRAM_MEMBERSHIP_VALUE_QA.md",
    "V870_SENTINEL_EMPLOYEE_WORKFLOW_QA.md",
    "V870_RELEASE_CLEANER_HARDENING_QA.md",
    "V870_NEXT_PRO_MAX_REFERENCE_STEPS.md",
]
...
def fail(message: str) -> None:
    raise SystemExit(f"V870 PRO MAX check FAILED: {message}")
...
def main() -> None:
    ...
    require(read("VERSION.txt").strip() == VERSION, "VERSION.txt is not V870 PRO MAX")
    require(read("APP_VERSION").strip() == VERSION, "APP_VERSION is not V870 PRO MAX")
    require(f"APP_VERSION = '{VERSION}'" in app_py, "app.py APP_VERSION is not V870 PRO MAX")
    require(VERSION in base, "base.html missing V870 PRO MAX cache/version")
    require('data-v870-shell="true"' in base, "base.html missing data-v870-shell")
    require("NEMESIS V870 REFERENCE STYLE MATCH AND WORKSPACE PURGE PRO MAX ACTIVE" in base, "base.html missing V870 PRO MAX comment")
    require("has_v870_reference_style_match_workspace_purge" in app_py, "runtime V870 flag missing")
    require("V870 REFERENCE STYLE MATCH AND WORKSPACE PURGE PRO MAX START" in css, "CSS V870 PRO MAX marker missing")
    require("V870 REFERENCE STYLE MATCH AND WORKSPACE PURGE PRO MAX END" in css, "CSS V870 PRO MAX end marker missing")
    ...
    for suffix in ['".zip"', '".db"', '".sqlite"', '".mp4"', '".log"', '".pyc"']:
        require(suffix in build or suffix in audit, f"release/audit suffix guard missing: {suffix}")
    require("reports/V870_" in build and "RELEASE_ZIP_AUDIT_V870" in build, "build cleaner does not include V870 reports/audits")
    require("V870 PRO MAX workspace purge guardrails" in gitignore, ".gitignore missing V870 guardrails")
    ...
    os.environ.setdefault("DB_PATH", str(ROOT / "tmp_v870_pro_max_runtime_check.sqlite"))
    os.environ.setdefault("AUTOMATION_SECRET", "codex-v870-pro-max-check")
    ...
    require(payload.get("app_version") == VERSION, "runtime app_version not V870 PRO MAX")
    require(payload.get("version_txt") == VERSION, "runtime version_txt not V870 PRO MAX")
    ...
    require(
        client.get("/api/automation/master-tick?dry_run=1&secret=codex-v870-pro-max-check").status_code == 200,
        "master tick with secret is not 200",
    )
    ...
    if zip_path.exists():
        with zipfile.ZipFile(zip_path) as zf:
            ...
            for report in REPORTS:
                require(f"reports/{report}" in names, f"ZIP missing report {report}")

    print("V870 PRO MAX reference style match and workspace purge OK")

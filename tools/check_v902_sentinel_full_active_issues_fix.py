...
def main() -> int:
    ...
    require(read("VERSION.txt").strip().lstrip("\ufeff") == VERSION, "VERSION.txt is not V902", failures)
    require(read("APP_VERSION").strip().lstrip("\ufeff") == VERSION, "APP_VERSION file is not V902", failures)
    require(app_version_from_source(app_py) == VERSION, "app.py APP_VERSION is not V902", failures)
    require("NEMESIS_CACHE_V902" in app_py, "service worker cache V902 missing", failures)
    ...
    require(runtime.get("app_version") == VERSION, "runtime app_version not V902", failures)

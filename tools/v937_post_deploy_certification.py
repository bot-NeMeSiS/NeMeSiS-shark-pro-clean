"""Read-only Render deployment and sports evidence certification."""
...
if ACTIVE_VERSION_TAG is None:
    raise RuntimeError("VERSION.txt does not start with a canonical V-number")
try:
    MADRID = ZoneInfo("Europe/Madrid")
except Exception:  # Windows runners may not ship the IANA database.
    MADRID = datetime.now().astimezone().tzinfo
EXPECTED_CACHE = f"NEMESIS_CACHE_{ACTIVE_VERSION_TAG.group(1)}"
PUBLIC_ROUTES = {
    "/": 200,
    "/calendar": 200,
    "/live": 200,
...
def _request(base_url: str, path: str, timeout: int = 30) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "NeMeSiS-Post-Deploy-Certifier/2.0",
...
def main() -> int:
    ...
    if args.expected_version != ACTIVE_VERSION:
        raise SystemExit("expected version must match the checked-out release identity")

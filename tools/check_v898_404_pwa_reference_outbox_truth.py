...
def main() -> int:
    ...
    require(read("VERSION.txt").strip().lstrip("\ufeff") == VERSION, "VERSION.txt is not V898", failures)
    require(read("APP_VERSION").strip().lstrip("\ufeff") == VERSION, "APP_VERSION file is not V898", failures)
    require(app_version_from_source(app_py) == VERSION, "app.py APP_VERSION is not V898", failures)
    require(VERSION in base and "data-v898-shell" in base, "base V898 marker missing", failures)
    ...
    require("NEMESIS_CACHE_V898" in app_py, "service worker cache V898 missing", failures)
    ...
    require(sw.status_code == 200 and "NEMESIS_CACHE_V898" in sw_text, "service worker V898 unavailable", failures)

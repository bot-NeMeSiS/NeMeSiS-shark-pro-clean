...
def main() -> int:
    ...
    require(local_version == VERSION, "VERSION.txt is not V920", failures)
    require(read("APP_VERSION").strip().lstrip("\ufeff") == VERSION, "APP_VERSION is not V920", failures)
    require(app_version(app_py) == VERSION, "app.py APP_VERSION is not V920", failures)
    require("NEMESIS_CACHE_V920" in app_py, "service worker cache V920 missing", failures)
    for flag in [
    ...
    require(payload.get("version") == VERSION, "runtime version is not V920", failures)

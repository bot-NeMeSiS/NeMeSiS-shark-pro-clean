...
def main() -> None:
    ...
    os.environ.setdefault("DB_PATH", str(Path(tempfile.gettempdir()) / "nemesis_v857_smoke.sqlite"))
    os.environ.setdefault("AUTOMATION_SECRET", "codex-v857-smoke")
    ...
    routes = [
        ...
        "/admin/dashboard", "/admin/company-os", "/admin/control-center",
        "/admin/data-center", "/admin/api-sports", "/admin/api-sports-audit",
    ...
    with_secret = client.get("/api/automation/master-tick?dry_run=1&secret=codex-v857-smoke")
    if with_secret.status_code != 200:
        failures.append(f"master-tick con secret dry_run -> {with_secret.status_code}")

    health = client.get("/api/automation/health-check?secret=codex-v857-smoke")
    ...
    if runtime.status_code != 200:
        ...
        if data.get("app_version") != "V857_COMPANY_OPERATING_SYSTEM_PRODUCT_PERFECTION_FINAL":
            failures.append("runtime app_version no es V857")
        if not data.get("has_v857_company_os"):
            failures.append("runtime flag V857 ausente")
        for flag in ["has_v856_real_app_reference_gap_second_pass", "has_v855_full_ecosystem_reference_rebuild", "has_v818_automation"]:
            if not data.get(flag):
                failures.append(f"runtime flag preservado ausente: {flag}")

    if failures:
        raise SystemExit("V857 smoke failed:\n- " + "\n- ".join(failures))
    print("V857 Flask smoke OK")

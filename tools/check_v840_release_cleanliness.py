...
with zipfile.ZipFile(zip_arg) as z: names=z.namelist()
forbidden=[]
for n in names:
    low=n.lower()
    if low in ['.env.example','.env.render.clean']: continue
    if low.endswith('.zip') or any(tok in low for tok in forbidden_tokens): forbidden.append(n)

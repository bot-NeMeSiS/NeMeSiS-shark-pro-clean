...
def main() -> int:
    ...
    require(version_bytes.decode("utf-8").strip() == VERSION, "VERSION.txt is not V911 video fix", failures)
    require(read("APP_VERSION").strip().lstrip("\ufeff") == VERSION, "APP_VERSION is not V911 video fix", failures)
    require(app_version_from_source(app_py) == VERSION, "app.py APP_VERSION is not V911 video fix", failures)
    ...
    require("NEMESIS_CACHE_V911" in app_py, "service worker cache V911 missing", failures)
    ...
    require(runtime.get("version") == VERSION, "runtime version is not V911 video fix", failures)
    ...
    require(sw.status_code == 200 and "NEMESIS_CACHE_V911" in sw.get_data(as_text=True), "service worker cache V911 not served", failures)

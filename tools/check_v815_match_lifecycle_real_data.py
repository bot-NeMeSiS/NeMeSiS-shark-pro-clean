...
def main() -> None:
    os.environ.setdefault("DB_PATH", str(ROOT / "data" / "v815_lifecycle_check.db"))
    ...
    cases = {
        "futuro": (
            {"match_date": (now + timedelta(days=1)).date().isoformat(), "kickoff_time": "21:00", "status": "NS"},
            "UPCOMING",
        ),
        "empezado_sin_score": (
            {"kickoff_iso": (now - timedelta(minutes=35)).isoformat(), "status": "NS"},
            "LIVE_PENDING",
        ),
        "live": (
            {"match_date": now.date().isoformat(), "kickoff_time": (now - timedelta(minutes=12)).strftime("%H:%M"), "status": "1H", "minute": "12", "score": "1-0"},
            "LIVE",
        ),
        "finalizado": (
            {"match_date": (now - timedelta(days=1)).date().isoformat(), "kickoff_time": "20:00", "status": "FT", "score": "2-1"},
            "FT",
        ),
        "madrugada_pasada": (
            {"match_date": (now - timedelta(days=1)).date().isoformat(), "kickoff_time": "00:30", "status": "NS"},
            "RESULT_PENDING",
        ),
        "sin_score_api": (
            {"match_date": (now - timedelta(days=2)).date().isoformat(), "kickoff_time": "18:00", "status": ""},
            "RESULT_PENDING",
        ),
    }
    results = {}
    ...
    if failed:
        fail("Fallan casos de ciclo de vida V815: " + ", ".join(failed), results)

  ...
  function createDirectoController(root) {
    ...
    function accept(id, state, serverNow) {
      ...
      records.set(id, {state: state, evaluated: evaluated, observed: observed, budget: budget,
        mono: performance.now(), wall: Date.now(), withheld: withheld});
      return true;
    }
    ...
    function draw(wrapper, entry) {
      ...
      text(article.querySelector('[data-v934-score]'), hasScore && status !== 'UPCOMING'
      ...
      if (meta) {
        ...
        if (labelNode && labelNode.nodeType === Node.TEXT_NODE) {
          labelNode.textContent = (isLive ? 'Observación vigente' : expired ? 'Datos retrasados · Última lectura' :
        ...
        if (timeNode && Number.isFinite(stamp(state.provider_observed_at))) {
          ...
          text(timeNode, new Intl.DateTimeFormat('es-ES', {timeZone:'Europe/Madrid', day:'2-digit', month:'2-digit',
      ...
      article.querySelectorAll('.v937-confidence-badge').forEach(function (badge) {
        ...
        badge.setAttribute('aria-label', 'Confianza del dato: ' + (isLive ? 'observación vigente' : label));
        badge.title = 'Estado de la observación. No mide probabilidad de ganar.';
      ...
      if (detail) {
        detail.hidden = !outside;
        text(detail, outside ? 'Fuera del filtro actual. Se conserva la última lectura y el acceso al partido.' : '');
      }

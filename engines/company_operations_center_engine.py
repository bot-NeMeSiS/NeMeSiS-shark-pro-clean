...
def _build_operations_sections(*, root_path: Path, app_version: str, version_txt: str, app_version_file: str, git: dict[str, Any], db_status: dict[str, Any], dr: dict[str, Any], privacy: dict[str, Any], systems: list[dict[str, Any]], scores: dict[str, Any], sports_metrics: dict[str, Any], cron_record: dict[str, Any], sports_record: dict[str, Any], telegram_record: dict[str, Any], external_runtime: dict[str, Any], external_certified: bool, runtime_aligned: bool, sports_provider: bool, telegram_configured: bool, stripe_configured: bool, secret_guard_present: bool) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    sections = {
        ...
        "sports_core": _section(
            "sports_core",
            "Sports Core",
            "PASS",
            "CONFIRMADO",
            "Contratos, relevancia y calidad deportiva integrados con evidencia local/cache-only.",
            items=[
                _item("Contrato", contracts.get("contract"), "CONFIRMADO", "sports_platform_contracts"),
                _item("Capacidades", len(contracts.get("capabilities") or []), "CONFIRMADO", "sports_platform_contracts"),
                _item("LIVE real", (sports_metrics.get("sports_quality") or {}).get("live_real", 0), "CONFIRMADO", "MATCH-STATUS-TRUTH-V1"),
                _item("Conflictos LIVE", (sports_metrics.get("sports_quality") or {}).get("live_conflicts", 0), "CONFIRMADO", "MATCH-STATUS-TRUTH-V1"),
                _item("Competiciones sin mapear", (sports_metrics.get("sports_quality") or {}).get("unknown", 0), "CONFIRMADO", "SPORTS-UNKNOWN-QUALITY-V1"),
                _item("Partidos stale", (sports_metrics.get("sports_quality") or {}).get("stale", 0), "CONFIRMADO", "sports-metrics-v1"),
                _item("Tier S/A disponibles", (sports_metrics.get("sports_quality") or {}).get("tier_sa_available", 0), "CONFIRMADO", "sports-relevance-v2"),
                _item("Tier S/A mostrados", (sports_metrics.get("sports_quality") or {}).get("tier_sa_surfaced", 0), "CONFIRMADO", "sports-relevance-v2"),
                _item("Ultima sincronizacion", (sports_metrics.get("sports_quality") or {}).get("last_sync") or sports_metrics.get("last_sync") or "No disponible", "CONFIRMADO", "DB/cache local"),
            ],
            next_action="Mantener DAY 2-7 y certificar un partido Tier S/A realmente LIVE.",
        ),
...
def _safe_operations_actions() -> list[dict[str, Any]]:
    return [
        ...
        {"key": "open_sentinel", "label": "Abrir Sentinel", "kind": "link", "href": "/admin/sentinel-autopilot", "description": "Revisa tareas y evidencias sin autocorregir código.", "requires_approval": False, "dangerous": False},
...
def build_company_operations_snapshot(
    ...
    systems = [
        ...
        _system("shark", "SHARK", "READY", "CONFIRMADO", "Motor y guardas locales presentes; produccion sigue pendiente de medicion externa.", "código y benchmark local V937", "medium", {"openai_configured": _env_present("OPENAI_API_KEY"), "production_certified": False}, "Medir produccion y confirmar cero escrituras por GET."),

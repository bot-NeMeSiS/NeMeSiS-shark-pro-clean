...
def build_real_match(event, sport_key):
    ...
    match = {
        ...
        "home_logo": event.get("strHomeTeamBadge") or event.get("home_logo") or f"/team-badge.svg?{urllib.parse.urlencode({'name': home or 'Equipo'})}",
        "away_logo": event.get("strAwayTeamBadge") or event.get("away_logo") or f"/team-badge.svg?{urllib.parse.urlencode({'name': away or 'Rival'})}",
        "live_score": event.get("live_score") or "",
        "live_minute": event.get("live_minute") or "",
...
def validate_match(match):
    ...
    demo_text = " ".join([home, away, league, str(match.get("date_full") or ""), str(match.get("commence_time") or "")]).lower()
    if any(term in demo_text for term in ["demo", "test", "mock", "example", "equipo local", "equipo visitante", "team a", "team b"]):
        score -= 80
        reasons.append("contenido fake/legacy bloqueado")

    if source not in {"the_odds_api", "the_sportsdb", "api-football", "real"}:
        score -= 35
...
def build_livescore_match(event):
    home = clean_text(event.get("strHomeTeam") or event.get("homeTeam") or event.get("strHome"))
    away = clean_text(event.get("strAwayTeam") or event.get("awayTeam") or event.get("strAway"))
    if not home or not away:
        return None
    hs = event.get("intHomeScore") if event.get("intHomeScore") is not None else event.get("homeScore")
    aw = event.get("intAwayScore") if event.get("intAwayScore") is not None else event.get("awayScore")
    score = f"{hs} - {aw}" if str(hs or "") != "" and str(aw or "") != "" else ""
    progress = clean_text(event.get("strProgress") or event.get("strStatus") or event.get("intMinute") or "")
    raw_status = progress.upper()
    if "FINAL" in raw_status or "FT" in raw_status:
        status = "FINALIZADO"
    elif "HT" in raw_status or "HALF" in raw_status:
        status = "DESCANSO"
    else:
        status = "EN DIRECTO"
    now = utc_now().isoformat()
    league = clean_text(event.get("strLeague") or event.get("league") or event.get("strSport")) or "Live"
    match = {
        "id": str(event.get("idEvent") or event.get("idLiveScore") or stable_id(home, away, now, league)),
        "source": "the_sportsdb",
        "provider": "TheSportsDB",
        "sport_key": clean_text(event.get("strSport") or "Soccer"),
        "league": league,
        "home_team": home,
        "away_team": away,
        "title": f"{home} vs {away}",
        "home_logo": event.get("strHomeTeamBadge") or f"/team-badge.svg?{urllib.parse.urlencode({'name': home})}",
        "away_logo": event.get("strAwayTeamBadge") or f"/team-badge.svg?{urllib.parse.urlencode({'name': away})}",
        "live_score": score,
        "live_minute": progress,
        "commence_time": now,
        "iso": parse_datetime(now).isoformat() if parse_datetime(now) else now,
        "date": "LIVE",
        "date_full": local_now().strftime("%d/%m/%Y"),
        "time": local_now().strftime("%H:%M"),
        "status": status,
        "relative": progress or "En vivo",
        "market": "Seguimiento live",
        "selection": None,
        "odds": None,
        "bookmaker": None,
        "real": True,
        ...
        "stake": "Vigilar",
        "valid": True,
        "quality_score": 100 if score else 88,
        "reject_reasons": ["OK"],
        "confidence_score": 82,
        "shark_score": 82,
    }
...
def fetch_thesportsdb_livescore():
    if requests is None:
        return []
    key = env_first("THESPORTSDB_KEY")
    if not key:
        return []
    sport = os.getenv("THESPORTSDB_SPORT", "Soccer") or "Soccer"
    timeout = int(os.getenv("HTTP_TIMEOUT_SECONDS", "8"))
    try:
        response = requests.get(
            f"https://www.thesportsdb.com/api/v2/json/livescore/{urllib.parse.quote(sport)}",
...
class RealCoreEngine:
    ...
    def _fetch_the_odds_api():
        ...
        # V442: añade livescore real con escudos/marcadores cuando TheSportsDB está activo.
        try:
            matches.extend(fetch_thesportsdb_livescore())
        except Exception:
            pass

        matches = dedupe(matches)
        matches.sort(key=lambda m: (0 if m.get("status") == "EN DIRECTO" else 1, m.get("iso") or ""))

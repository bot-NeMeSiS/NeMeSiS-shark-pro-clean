...
from engines.autonomous_company_sentinel_engine import (
...
def service_worker():
    body = (
        "const NEMESIS_CACHE='NEMESIS_CACHE_V919';\n"
...
def _v894_company_sentinel_runtime_state():
    runtime = _v893_autonomous_runtime_state()
    runtime["local_app_version"] = APP_VERSION
    return runtime


def _v894_run_company_sentinel(mode="safe_scan", runner="local", dry_run=True):
    visual = run_visual_company_worker(app.test_client(), APP_VERSION, mode="visual" if mode == "visual_scan" else "quick", dry_run=True)
    autopilot = _v888_build_autopilot_scan(save_memory=False, mode="quick")
    return run_autonomous_company_sentinel(
        app.test_client(),
        APP_VERSION,
        Path(__file__).resolve().parent,
        mode=mode,
        runner=runner,
        dry_run=dry_run,
        runtime=_v894_company_sentinel_runtime_state(),
        visual_result=visual,
        autopilot_result=autopilot,
    )


@app.route("/admin/autonomous-company-sentinel")
@app.route("/admin/company-sentinel")
@app.route("/admin/auto-qa")
@app.route("/admin/sentinel-empresa")
...
@app.route("/admin/codex-outbox")
@app.route("/admin/prompts-codex")
def admin_sentinel_codex_outbox_page():
    if not is_admin_session():
...
def v915_admin_workforce_status() -> dict:
    ...
    workers = [
        ...
        {"name": "Post-Deploy Sentinel", "file": "post_deploy_sentinel.py", "status": summary.get("v915_post_deploy_sentinel_status")},
    ...
    workflows = [
        {"name": "CI QA", "path": ".github/workflows/nemesis-ci.yml", "status": "ready" if (root / ".github" / "workflows" / "nemesis-ci.yml").exists() else "missing"},
        {"name": "Render Deploy", "path": ".github/workflows/render-deploy.yml", "status": "ready" if (root / ".github" / "workflows" / "render-deploy.yml").exists() else "missing"},
        {"name": "Browser QA", "path": ".github/workflows/browser-qa.yml", "status": "ready" if (root / ".github" / "workflows" / "browser-qa.yml").exists() else "missing"},
    ]
    return {
        "version": APP_VERSION,
        ...
        "workers": workers,
        "workflows": workflows,
        "next_action": browser_gate.get("v919_next_required_action") or post_deploy.get("v918_next_required_action") or summary.get("v915_next_required_action"),
        "policy": {
            "automated_deploy_requires_env": "ENABLE_AUTOMATED_RENDER_DEPLOY=1",
            "secrets_visible": False,
            "telegram_real_send": False,
            "payments_real_touch": False,
        },
    }


@app.route("/admin/automation-workforce")
@app.route("/admin/workforce")
@app.route("/admin/company-workers")
@app.route("/admin/deploy-center")
...
def api_admin_automation_workforce_status():
    if not is_admin_session():
        return admin_json_forbidden()
    return jsonify({"ok": True, "version": APP_VERSION, "workforce": v915_admin_workforce_status()})


@app.route("/api/admin/automation-workforce/run-checks", methods=["POST"])
def api_admin_automation_workforce_run_checks():
    if not is_admin_session():
        return admin_json_forbidden()
    from automation_workforce.release_manager import run_release_manager
    result = run_release_manager(dry_run=True)
    return jsonify({"ok": bool(result.get("ok", True)), "version": APP_VERSION, "result": result})


@app.route("/api/admin/automation-workforce/verify-runtime", methods=["POST"])
def api_admin_automation_workforce_verify_runtime():
    if not is_admin_session():
        return admin_json_forbidden()
    from automation_workforce.runtime_verifier import run_runtime_verifier
    result = run_runtime_verifier(dry_run=True)
    return jsonify({"ok": bool(result.get("ok", True)), "version": APP_VERSION, "result": result})


@app.route("/api/admin/automation-workforce/post-deploy-sentinel", methods=["POST"])
def api_admin_automation_workforce_post_deploy_sentinel():
    if not is_admin_session():
        return admin_json_forbidden()
    from automation_workforce.post_deploy_sentinel import run_post_deploy_sentinel
    result = run_post_deploy_sentinel(dry_run=True)
    return jsonify({"ok": bool(result.get("ok", True)), "version": APP_VERSION, "result": result})


@app.route("/api/admin/automation-workforce/browser-qa-status", methods=["POST"])
def api_admin_automation_workforce_browser_qa_status():
    if not is_admin_session():
        return admin_json_forbidden()
    from automation_workforce.browser_qa_orchestrator import run_browser_qa_orchestrator
    result = run_browser_qa_orchestrator(dry_run=True)
    return jsonify({"ok": bool(result.get("ok", True)), "version": APP_VERSION, "result": result})


@app.route("/api/admin/automation-workforce/visual-queue-refresh", methods=["POST"])
def api_admin_automation_workforce_visual_queue_refresh():
    if not is_admin_session():
        return admin_json_forbidden()
    from automation_workforce.visual_queue_manager import run_visual_queue_manager
    result = run_visual_queue_manager(dry_run=True)
    return jsonify({"ok": bool(result.get("ok", True)), "version": APP_VERSION, "result": result})
...
@app.route("/api/admin/autonomous-company-sentinel/status")
def api_admin_autonomous_company_sentinel_status():
    if not is_admin_session():
        return admin_json_forbidden()
    return jsonify({"ok": True, **build_company_sentinel_status(APP_VERSION, Path(__file__).resolve().parent)})


@app.route("/api/admin/autonomous-company-sentinel/latest-run")
def api_admin_autonomous_company_sentinel_latest_run():
    if not is_admin_session():
        return admin_json_forbidden()
    return jsonify({"ok": True, "version": APP_VERSION, "latest_run": build_company_sentinel_status(APP_VERSION, Path(__file__).resolve().parent).get("latest_run")})


@app.route("/api/admin/autonomous-company-sentinel/issues")
def api_admin_autonomous_company_sentinel_issues():
    if not is_admin_session():
        return admin_json_forbidden()
    status = build_company_sentinel_status(APP_VERSION, Path(__file__).resolve().parent)
...
@app.route("/api/admin/autonomous-company-sentinel/outbox")
def api_admin_autonomous_company_sentinel_outbox():
    if not is_admin_session():
        return admin_json_forbidden()
    outbox_path = Path(__file__).resolve().parent / "data" / "runtime" / "autonomous_company_sentinel" / "codex_outbox.md"
...
@app.route("/api/admin/autonomous-company-sentinel/autofix-plan")
def api_admin_autonomous_company_sentinel_autofix_plan():
    if not is_admin_session():
        return admin_json_forbidden()
    plan_path = Path(__file__).resolve().parent / "data" / "runtime" / "autonomous_company_sentinel" / "autofix_plan.json"
...
@app.route("/api/admin/autonomous-company-sentinel/reference-gaps")
def api_admin_autonomous_company_sentinel_reference_gaps():
    if not is_admin_session():
        return admin_json_forbidden()
    path = Path(__file__).resolve().parent / "data" / "runtime" / "autonomous_company_sentinel" / "reference_gap_report.json"
    data = json.loads(path.read_text(encoding="utf-8")) if path.exists() else {"issues": [], "reference_count": 0}
    return jsonify({"ok": True, "version": APP_VERSION, "reference_gaps": data})


@app.route("/api/admin/autonomous-company-sentinel/render-alignment")
def api_admin_autonomous_company_sentinel_render_alignment():
    if not is_admin_session():
        return admin_json_forbidden()
    path = Path(__file__).resolve().parent / "data" / "runtime" / "autonomous_company_sentinel" / "render_alignment.json"
    data = json.loads(path.read_text(encoding="utf-8")) if path.exists() else {"aligned": False, "render_version": "No consultado"}
...
def api_admin_autonomous_company_sentinel_run():
    ...
    result = _v894_run_company_sentinel(mode=mode, runner="admin", dry_run=dry_run)
    return jsonify({"ok": True, **result})


@app.route("/api/admin/autonomous-company-sentinel/generate-codex-prompts", methods=["GET", "POST"])
@app.route("/api/admin/autonomous-company-sentinel/export-outbox", methods=["GET", "POST"])
def api_admin_autonomous_company_sentinel_generate_codex_prompts():
    if not is_admin_session():
        return admin_json_forbidden()
    result = _v894_run_company_sentinel(mode="autofix_plan", runner="admin", dry_run=True)
...
def api_admin_autonomous_company_sentinel_sync_issues():
    if not is_admin_session():
        return admin_json_forbidden()
    result = _v894_run_company_sentinel(mode="safe_scan", runner="admin", dry_run=True)
...
def v906_browser_qa_runtime_summary() -> dict:
    root = Path(__file__).resolve().parent
    comparison_path = root / "data" / "runtime" / "autonomous_company_sentinel" / "browser_reference_comparison.json"
    status_path = root / "reports" / "V906_browser_qa" / "browser_reference_qa_report.json"
    summary = {
        "v906_browser_qa_available": False,
        "v906_screenshots_captured": 0,
        "v906_reference_comparisons": 0,
        "v906_visual_gaps_resolved": 0,
        "v906_visual_gaps_pending": 0,
        "v906_browser_qa_status": "BROWSER_QA_UNAVAILABLE",
    }
    try:
        if comparison_path.exists():
            payload = json.loads(comparison_path.read_text(encoding="utf-8"))
            status = str(payload.get("browser_qa_status") or summary["v906_browser_qa_status"])
            summary.update({
                "v906_browser_qa_available": status.upper() == "CAPTURED",
                "v906_screenshots_captured": int(payload.get("screenshots_captured") or 0),
                "v906_reference_comparisons": int(payload.get("reference_comparisons") or 0),
                "v906_visual_gaps_resolved": int(payload.get("visual_gaps_resolved") or 0),
                "v906_visual_gaps_pending": int(payload.get("visual_gaps_pending") or 0),
                "v906_browser_qa_status": status,
            })
        elif status_path.exists():
            payload = json.loads(status_path.read_text(encoding="utf-8"))
            summary.update({
                "v906_browser_qa_available": bool(payload.get("browser_available")),
                "v906_screenshots_captured": int(payload.get("screenshots_captured") or 0),
                "v906_browser_qa_status": "CAPTURED" if payload.get("browser_available") else "BROWSER_QA_UNAVAILABLE",
            })
    except Exception:
        pass
    return summary
...
def v909_browser_qa_pipeline_runtime_summary() -> dict:
    root = Path(__file__).resolve().parent
    browser_dir = root / "browser_qa"
    workflow_path = root / ".github" / "workflows" / "browser-qa.yml"
    workflow_example_path = root / "docs" / "browser_qa_github_action_example.yml"
    ...
    return {
        "v909_browser_qa_pipeline_ready": (
            (browser_dir / "README.md").exists()
            and (browser_dir / "run_local_browser_qa.ps1").exists()
            and (browser_dir / "run_local_browser_qa.bat").exists()
            and (browser_dir / "run_local_browser_qa.sh").exists()
            and (browser_dir / "playwright_requirements.txt").exists()
        ),
        "v909_visual_fix_queue_count": len(items),
        "v909_blocked_no_screenshot_count": len(blocked),
        "v909_github_action_available": workflow_path.exists() or workflow_example_path.exists(),
        "v909_local_runner_available": (browser_dir / "run_local_browser_qa.ps1").exists() and (browser_dir / "playwright_requirements.txt").exists(),
    }
...
def v912_video_admin_ui_copy_polish_runtime_summary() -> dict:
    ...
    return {
        **v911,
        ...
        "v912_pwa_route_recheck_status": "service_worker_v912_or_newer_no_404_cache" if ("NEMESIS_CACHE_V912" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V913" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V915" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V916" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V917" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V918" in Path(__file__).read_text(encoding="utf-8", errors="replace") or "NEMESIS_CACHE_V919" in Path(__file__).read_text(encoding="utf-8", errors="replace")) else "pending_cache_update",
    }


def v913_browser_qa_runtime_truth_summary() -> dict:
    """Runtime-safe truth summary for Browser QA execution/import state."""
    root = Path(__file__).resolve().parent
    runtime_dir = root / "data" / "runtime" / "autonomous_company_sentinel"
    reports_dir = root / "reports"
    queue_path = runtime_dir / "visual_fix_queue.json"
    ...
    blocked = [item for item in items if isinstance(item, dict) and item.get("status") == "BLOCKED_NO_SCREENSHOT"]
    ready = [item for item in items if isinstance(item, dict) and item.get("status") in {"READY_FOR_CODEX", "FIXABLE_SAFE"}]
    invalid = [item for item in items if isinstance(item, dict) and item.get("status") not in valid_statuses]
    screenshots = int(status_payload.get("screenshots_captured") or comparison_payload.get("screenshots_captured") or 0)
    comparisons = int(comparison_payload.get("reference_comparisons") or 0)
    browser_status = str(status_payload.get("browser_qa_status") or comparison_payload.get("browser_qa_status") or "BROWSER_QA_UNAVAILABLE")
    reports_expected = [
        "V913_BROWSER_QA_EXECUTION_STATUS_TRUTH_REPORT.md",
        "V913_CURRENT_RUNTIME_AND_BROWSER_QA_STATE_AUDIT.md",
        "V913_RUNTIME_STATUS_TRUTH_CLEANUP_QA.md",
        "V913_BROWSER_QA_EXECUTION_OR_BLOCKER_STATUS.md",
        "V913_VISUAL_FIX_QUEUE_TRUTH_QA.md",
        "V913_CODEX_OUTBOX_TRUTH_QA.md",
        "V913_ADMIN_PANEL_CLARITY_QA.md",
        "V913_PWA_ROUTE_RECHECK_QA.md",
        "V913_NEXT_STEPS.md",
    ]
    reports_ready = all((reports_dir / name).exists() for name in reports_expected)
    outbox_text = outbox_path.read_text(encoding="utf-8", errors="replace") if outbox_path.exists() else ""
    return {
        "v913_browser_qa_status": browser_status,
        "v913_screenshots_available": screenshots > 0,
        "v913_screenshots_captured": screenshots,
        "v913_visual_queue_total": len(items),
        "v913_visual_queue_blocked": len(blocked),
        "v913_visual_queue_ready": len(ready),
        "v913_visual_queue_invalid_status_count": len(invalid),
        "v913_reference_comparisons": comparisons,
        "v913_runtime_status_cleanup_applied": True,
        "v913_reports_ready": reports_ready,
        "v913_outbox_truth_sections_ready": "V913_BROWSER_QA_EXECUTION_REQUIRED" in outbox_text,
        "v913_pixel_perfect_claim_allowed": bool(screenshots and comparisons and not blocked and comparison_payload.get("pixel_perfect_claim") is True),
        "v913_next_required_action": "run_browser_qa_or_import_results" if screenshots == 0 else "review_ready_visual_fix_queue",
    }


def v915_automated_workforce_runtime_summary() -> dict:
    """Runtime-safe V915/V916 summary for the automated company workforce."""
    root = Path(__file__).resolve().parent
    workforce_dir = root / "automation_workforce"
    ...
    workers_ready = all((workforce_dir / name).exists() for name in required_workers)
    ci_ready = (workflows_dir / "nemesis-ci.yml").exists()
    deploy_ready = (workflows_dir / "render-deploy.yml").exists()
    browser_action_ready = (workflows_dir / "browser-qa.yml").exists()
    browser_ready = (root / "browser_qa" / "README.md").exists() and (root / "tools" / "check_browser_qa_environment.py").exists()
    deploy_enabled = env_bool("ENABLE_AUTOMATED_RENDER_DEPLOY", False)
    try:
        queue_payload = json.loads(queue_path.read_text(encoding="utf-8-sig", errors="replace")) if queue_path.exists() else {"items": []}
    except Exception:
        queue_payload = {"items": []}
    try:
        browser_status = json.loads(status_path.read_text(encoding="utf-8-sig", errors="replace")) if status_path.exists() else {}
    except Exception:
        browser_status = {}
    ...
    hook_state = mask_secret(os.getenv("RENDER_DEPLOY_HOOK_URL"))
    api_state = mask_secret(os.getenv("RENDER_API_KEY"))
    service_state = mask_secret(os.getenv("RENDER_SERVICE_ID"))
    deploy_hook_configured = hook_state == "***configured***"
    render_api_configured = api_state == "***configured***" and service_state == "***configured***"
    deploy_configured = deploy_hook_configured or render_api_configured
    deploy_status = "ready" if deploy_enabled and deploy_configured else ("missing_deploy_hook" if deploy_enabled else "dry_run_disabled")
    security_status = "ready" if (workforce_dir / "security_secret_guard.py").exists() else "missing"
    browser_status = browser_status.get("status") or browser_status.get("browser_qa_status") or "unknown"
    core_ready = workers_ready and ci_ready and deploy_ready and browser_action_ready and browser_ready and security_status == "ready"
    next_action = "configure_deploy_hook_and_run_browser_qa"
    if not deploy_configured:
        next_action = "configure_render_deploy_hook_secret"
    if browser_status in {"PACKAGE_MISSING", "BROWSERS_MISSING", "LAUNCH_FAILED", "unknown"}:
        next_action = "install_playwright_or_run_browser_qa_action"
    if blocked_no_screenshot:
        next_action = "run_browser_qa_then_refresh_visual_queue"
    return {
        "v915_workforce_ready": core_ready and deploy_configured and browser_status not in {"PACKAGE_MISSING", "BROWSERS_MISSING", "LAUNCH_FAILED", "unknown"} and not blocked_no_screenshot,
        "v916_workforce_core_ready": core_ready,
        "v916_automated_deploy_configured": bool(deploy_configured),
        "v916_deploy_hook_configured": bool(deploy_hook_configured),
        "v916_browser_qa_runtime_available": browser_status not in {"PACKAGE_MISSING", "BROWSERS_MISSING", "LAUNCH_FAILED", "unknown"},
        "v916_browser_qa_github_action_ready": browser_action_ready,
        "v916_visual_queue_blocked_count": len(blocked_no_screenshot),
        "v916_visual_queue_ready_count": len([item for item in queue_items if isinstance(item, dict) and item.get("status") in {"READY_FOR_CODEX", "FIXABLE_SAFE"}]),
        "v916_post_deploy_sentinel_ready": (workforce_dir / "post_deploy_sentinel.py").exists(),
        "v916_secret_guard_ready": security_status == "ready",
        "v916_next_required_action": next_action,
        "v915_render_deploy_worker_status": deploy_status,
        "v915_runtime_verifier_status": "ready" if (workforce_dir / "runtime_verifier.py").exists() else "missing",
        "v915_post_deploy_sentinel_status": "ready" if (workforce_dir / "post_deploy_sentinel.py").exists() else "missing",
        "v915_browser_qa_orchestrator_status": "ready" if (workforce_dir / "browser_qa_orchestrator.py").exists() and browser_ready else "missing_browser_pipeline",
        "v915_visual_queue_manager_status": "ready" if (workforce_dir / "visual_queue_manager.py").exists() and queue_path.exists() else "ready_no_queue_file",
        "v915_security_secret_guard_status": security_status,
        "v915_automated_deploy_enabled": deploy_enabled,
        "v915_next_required_action": next_action,
        "v915_visual_queue_items": len(queue_items),
        "v915_visual_queue_blocked_no_screenshot": len(blocked_no_screenshot),
        "v915_browser_qa_status": browser_status,
        "v915_github_actions_ci_ready": ci_ready,
        "v915_github_actions_render_deploy_ready": deploy_ready,
        "v915_deploy_hook_state": hook_state,
        "v915_render_api_key_state": api_state,
        "v915_render_service_id_state": service_state,
    }
...
def v919_browser_qa_results_gate_runtime_summary() -> dict:
    """Runtime-safe Browser QA import validation and screenshot evidence gate."""
    root = Path(__file__).resolve().parent
    runtime_dir = root / "data" / "runtime" / "autonomous_company_sentinel"
    status_path = runtime_dir / "browser_qa_status.json"
    comparison_path = runtime_dir / "browser_reference_comparison.json"
    ...
    try:
        status = json.loads(status_path.read_text(encoding="utf-8-sig", errors="replace")) if status_path.exists() else {}
    except Exception:
        status = {}
    try:
        comparison = json.loads(comparison_path.read_text(encoding="utf-8-sig", errors="replace")) if comparison_path.exists() else {}
    except Exception:
        comparison = {}
    try:
        queue_payload = json.loads(queue_path.read_text(encoding="utf-8-sig", errors="replace")) if queue_path.exists() else []
    ...
    if not isinstance(queue_items, list):
        queue_items = []
    desktop_count = len([p for p in (results_dir / "desktop").glob("*") if p.is_file() and p.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"} and p.stat().st_size > 0]) if (results_dir / "desktop").exists() else 0
    mobile_count = len([p for p in (results_dir / "mobile").glob("*") if p.is_file() and p.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"} and p.stat().st_size > 0]) if (results_dir / "mobile").exists() else 0
    valid_screenshots = desktop_count + mobile_count
    results_json_found = (results_dir / "browser_qa_result.json").exists()
    comparison_found = (results_dir / "reference_comparison.json").exists() or (results_dir / "browser_reference_comparison.json").exists() or comparison_path.exists()
    if valid_screenshots:
        discovery_status = "RESULTS_WITH_SCREENSHOTS"
    elif results_json_found or comparison_found:
        discovery_status = "RESULTS_WITHOUT_SCREENSHOTS"
    else:
        discovery_status = "NO_RESULTS_FOUND"
    blocked = len([item for item in queue_items if isinstance(item, dict) and item.get("status") == "BLOCKED_NO_SCREENSHOT"])
    ready = len([item for item in queue_items if isinstance(item, dict) and item.get("status") in {"READY_FOR_CODEX", "FIXABLE_SAFE"}])
    invalid_ready = len([
        item for item in queue_items
        ...
        and not (item.get("screenshot_path") or item.get("screenshot"))
    ])
    import_status = status.get("v919_import_status") or ("VALID_SCREENSHOTS_IMPORTED" if valid_screenshots else "NO_VALID_SCREENSHOTS_TO_IMPORT")
    return {
        "v919_browser_qa_discovery_status": discovery_status,
        "v919_results_json_found": bool(results_json_found),
        "v919_reference_comparison_found": bool(comparison_found),
        "v919_desktop_screenshots_count": desktop_count,
        "v919_mobile_screenshots_count": mobile_count,
        "v919_valid_screenshots_count": valid_screenshots,
        "v919_import_status": import_status,
        "v919_visual_queue_total": len(queue_items),
        "v919_visual_queue_blocked": blocked,
        "v919_visual_queue_ready": ready,
        "v919_visual_queue_invalid_ready_without_screenshot": invalid_ready,
        "v919_pixel_perfect_claim_allowed": False,
        "v919_next_required_action": "review_ready_visual_queue" if valid_screenshots and ready else "run_browser_qa_or_upload_artifacts",
    }
...
def api_runtime_version():
    ...
    git_commit_hint = ""
    try:
        version_txt = clean_version_text((BASE_DIR / "VERSION.txt").read_text(encoding="utf-8"))
    except Exception:
        version_txt = ""
    try:
        app_version_file = clean_version_text((BASE_DIR / "APP_VERSION").read_text(encoding="utf-8"))
    except Exception:
        app_version_file = ""
    try:
    ...
    v919_summary = v919_browser_qa_results_gate_runtime_summary()
    return jsonify(sanitize_runtime_value({
        ...
        "has_v892_autonomous_company_sentinel": "autonomous_company_sentinel_engine" in app_py_text and "/api/automation/autonomous-company-sentinel/run" in app_py_text and "/admin/autonomous-company-sentinel" in app_py_text,
        "has_v892_reference_qa_worker": "sentinel_reference_visual_engine" in app_py_text and "/api/admin/autonomous-company-sentinel/reference-gaps" in app_py_text,
        "has_v892_codex_outbox": "sentinel_codex_outbox_engine" in app_py_text and "/admin/sentinel-codex-outbox" in app_py_text,
        ...
        "has_v898_404_pwa_reference_outbox_truth": "V898_PRODUCTION_404_PWA_REFERENCE_OUTBOX_TRUTH_FINAL" in app_py_text and ("NEMESIS_CACHE_V898" in app_py_text or "NEMESIS_CACHE_V900" in app_py_text or "NEMESIS_CACHE_V901" in app_py_text or "NEMESIS_CACHE_V902" in app_py_text or "NEMESIS_CACHE_V903" in app_py_text or "NEMESIS_CACHE_V904" in app_py_text or "NEMESIS_CACHE_V906" in app_py_text or "NEMESIS_CACHE_V907" in app_py_text or "NEMESIS_CACHE_V908" in app_py_text or "NEMESIS_CACHE_V909" in app_py_text or "NEMESIS_CACHE_V911" in app_py_text or "NEMESIS_CACHE_V912" in app_py_text or "NEMESIS_CACHE_V913" in app_py_text or "NEMESIS_CACHE_V915" in app_py_text or "NEMESIS_CACHE_V916" in app_py_text or "NEMESIS_CACHE_V917" in app_py_text or "NEMESIS_CACHE_V918" in app_py_text or "NEMESIS_CACHE_V919" in app_py_text) and "/admin/not-found-events" in app_py_text,
        ...
        "has_v905_bom_version_alignment_fix": "clean_version_text" in app_py_text and "data-v905-shell" in base_template,
        ...
        "has_v910_route_not_found_pwa_audit": ("NEMESIS_CACHE_V910" in app_py_text or "NEMESIS_CACHE_V911" in app_py_text or "NEMESIS_CACHE_V912" in app_py_text or "NEMESIS_CACHE_V913" in app_py_text or "NEMESIS_CACHE_V915" in app_py_text or "NEMESIS_CACHE_V916" in app_py_text or "NEMESIS_CACHE_V917" in app_py_text or "NEMESIS_CACHE_V918" in app_py_text or "NEMESIS_CACHE_V919" in app_py_text) and (Path(__file__).resolve().parent / "reports" / "V910_ROUTE_NOT_FOUND_PWA_CACHE_AUDIT.md").exists(),
        ...
        "has_v913_browser_qa_result_importer": (Path(__file__).resolve().parent / "tools" / "import_browser_qa_results.py").exists(),
        "has_v915_automated_company_workforce": (Path(__file__).resolve().parent / "automation_workforce" / "release_manager.py").exists() and "data-v915-workforce-shell" in base_template,
        "has_v915_render_deploy_pipeline": (Path(__file__).resolve().parent / "automation_workforce" / "render_deploy_guard.py").exists() and (Path(__file__).resolve().parent / ".github" / "workflows" / "render-deploy.yml").exists(),
        "has_v915_post_deploy_sentinel": (Path(__file__).resolve().parent / "automation_workforce" / "post_deploy_sentinel.py").exists(),
        "has_v915_browser_qa_orchestrator": (Path(__file__).resolve().parent / "automation_workforce" / "browser_qa_orchestrator.py").exists(),
        "has_v915_security_secret_guard": (Path(__file__).resolve().parent / "automation_workforce" / "security_secret_guard.py").exists(),
        "has_v916_workforce_activation": (Path(__file__).resolve().parent / "reports" / "V916_WORKFORCE_ACTIVATION_AUDIT.md").exists(),
...
def api_v894_autonomous_company_sentinel_run():
    ...
    dry_run = request.args.get("dry_run", "1").lower() in {"1", "true", "yes", "on"}
    try:
        result = _v894_run_company_sentinel(mode=mode, runner=runner, dry_run=dry_run)
        counts = result.get("issues_summary", {}).get("counts", {})
        return jsonify({
            "ok": True,
            **result,
            "issues_created": counts.get("open", 0),
            "issues_open": counts.get("open", 0),
            "critical": counts.get("critical", 0),
            "high": counts.get("high", 0),
            "codex_prompts_generated": (result.get("outbox") or {}).get("prompt_count", 0),
            "autofix_applied": 0,
            "autofix_planned": (result.get("autofix_plan") or {}).get("safe_autofix_count", 0),
        })
    except Exception as exc:
        return jsonify({
            "ok": False,
            "version": APP_VERSION,
            "internal_error": True,
            "safe_message": "Autonomous Company Sentinel no pudo completar el scan.",
